...
function ensureCurl() {
    if ! command -v curl &> /dev/null; then
        log "Error: curl not found. Please run 'avs create' first."
        exit 1
    fi
}
...
function ensureAge() {
    if ! command -v age &> /dev/null; then
        log "Error: age not found. Please run 'avs create' first."
        exit 1
    fi
}
...
function ensureOras() {
    if ! command -v oras &> /dev/null; then
        log "Error: oras not found. Please run 'avs create' first."
        exit 1
    fi
}
