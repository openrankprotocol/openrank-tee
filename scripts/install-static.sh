...
# Detect OS and architecture for static builds
detect_platform() {
    local os=""
    local arch=""
    ...
    case "$(uname -s)" in
        ...
        *)
            ...
            print_error "For other platforms, use: curl -sSL https://raw.githubusercontent.com/${REPO}/main/scripts/install.sh | bash"
            exit 1
            ;;
    esac
    ...
    case "$(uname -m)" in
        ...
        *)
            print_error "Unsupported architecture for static builds: $(uname -m)"
            print_error "Static builds are only available for x86_64 and aarch64"
            exit 1
            ;;
    esac

    echo "${BINARY_NAME}-${os}-${arch}-static"
}
...
check_glibc_compatibility() {
    if command_exists ldd; then
        ...
        if [ -n "$glibc_version" ]; then
            ...
            # Check if glibc is older than 2.32 (rough threshold where compatibility issues start)
            local major minor
            major=$(echo "$glibc_version" | cut -d. -f1)
            minor=$(echo "$glibc_version" | cut -d. -f2)

            if [ "$major" -lt 2 ] || { [ "$major" -eq 2 ] && [ "$minor" -lt 32 ]; }; then
                print_warning "Your glibc version ($glibc_version) is quite old."
                print_status "Static builds are recommended for better compatibility."
            fi
        fi
    fi
}
...
install_openrank_static() {
    local version="$1"
    local force_install="$2"

    print_status "Starting OpenRank CLI static installation..."
    print_status "Static builds have no external dependencies and work on any Linux distribution."

    # Check glibc compatibility (informational)
    check_glibc_compatibility
    ...
    if command_exists "$BINARY_NAME" && [ "$force_install" != "true" ]; then
        ...
        if [ "$current_version" = "$version" ]; then
            ...
            # Check if current installation is static
            if ldd "$INSTALL_DIR/$BINARY_NAME" 2>/dev/null | grep -q "not a dynamic executable\|statically linked"; then
                print_status "Current installation is already static"
            else
                print_status "Current installation is dynamic. Use --force to replace with static build"
            fi
            exit 0
        else
    ...
    print_status "Downloading static binary: $asset_name..."
    ...
    print_success "OpenRank CLI $version (static) installed successfully!"
    print_status "Binary location: $INSTALL_DIR/$BINARY_NAME"

    # Verify installation and check if it's static
    if command_exists "$BINARY_NAME"; then
        print_success "Installation verified. Run '$BINARY_NAME --help' to get started."

        # Verify it's actually static
        if ldd "$INSTALL_DIR/$BINARY_NAME" 2>/dev/null | grep -q "not a dynamic executable\|statically linked"; then
            print_success "Confirmed: Binary is statically linked (no external dependencies)"
        else
            print_warning "Binary appears to have dynamic dependencies. This might not be the static build."
        fi
    else
...
show_usage() {
    cat << EOF
OpenRank CLI Static Installation Script

This script installs the statically linked version of OpenRank CLI for Linux.
Static builds have no external dependencies and work on any Linux distribution,
...
EXAMPLES:
    $0                      # Install latest static version
    $0 --version v1.0.0     # Install specific static version
    $0 --force              # Force reinstall latest static version
    $0 --dir ~/.local/bin   # Install to custom directory

NOTES:
    - Static builds are only available for Linux (x86_64 and aarch64)
    - For other platforms, use the regular install script
    - Static builds are larger but have better compatibility
    - Use this if you encounter glibc version errors with regular builds
...
main() {
    ...
    install_openrank_static "$version" "$force_install"
}
